...
sources = Glob("src/*.cpp") + Glob("src/**/*.cpp") # type: ignore
...
lib_filename = "{}gdSystems{}{}".format(env.subst('$SHLIBPREFIX'), env["suffix"], env.subst('$SHLIBSUFFIX'))

# Creates a SCons target for the path with our sources.
library = env.SharedLibrary(
    "x/bin/{}".format(lib_filename),
    source=sources,
)
